...
def _get_backbone_stages(backbone):
    for attr in ('stages', 'blocks', 'features', 'layers'):
        seq = getattr(backbone, attr, None)
...
class _StageCapture:
    ...
    def __enter__(self):
        stages = _get_backbone_stages(self.model.backbone)
...
def show_stage_viz(model, sample_row, device: str, out_dir: Path,
    ...
    stages   = _get_backbone_stages(model.backbone)
    n_stages = len(stages)
    if n_stages == 0:
        return None
    selected = [i for i in [0, 1, n_stages // 2, n_stages - 1]
                if i < n_stages]
    ...
    stage_labels = {selected[0]: "Stage 1 – edges"}
    if len(selected) > 1:
        stage_labels[selected[1]] = "Stage 2 – textures"
    if len(selected) > 2:
        stage_labels[selected[len(selected) // 2]] = f"Stage {selected[len(selected) // 2]+1} – parts"
    stage_labels[selected[-1]] = "Stage last – semantics"
    ...
    for idx in selected:
        ...
        label = stage_labels.get(idx, f"Stage {idx + 1}")
