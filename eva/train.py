...
from attention_viz import (show_attention_grid, show_test_attention_combined,
                           show_layer_viz)
...
class CsvMetricsLogger:
    FIELDNAMES = (
        ['epoch', 'mode', 'lr', 'elapsed_s',
         'train_loss', 'train_f1', 'train_mean_acc', 'train_sub_acc',
         'val_loss',   'val_f1',   'val_mean_acc',   'val_sub_acc']
        + [f'val_f1_{l}'  for l in LABELS]
        + [f'val_auc_{l}' for l in LABELS]
        + [f'val_acc_{l}' for l in LABELS]
        + [f'thr_{l}'     for l in LABELS]
        + ['is_best']
    )
    ...
    def write(self, **kw):
        row = {k: kw[k] for k in ['epoch', 'mode']}
        row['lr']        = f"{kw['lr']:.2e}"
        row['elapsed_s'] = int(kw['elapsed'])
        for k in ['train_loss', 'val_loss']:
            row[k] = f"{kw[k]:.5f}"
        for k in ['train_f1', 'train_mean_acc', 'train_sub_acc',
                  'val_f1',   'val_mean_acc',   'val_sub_acc']:
            row[k] = f"{kw[k]:.4f}"
        row['is_best'] = int(kw['is_best'])
        for i, l in enumerate(LABELS):
            row[f'val_f1_{l}']  = f"{kw['va_f1p'][i]:.4f}"
            row[f'val_auc_{l}'] = f"{kw['va_auc'][i]:.4f}"
            row[f'val_acc_{l}'] = f"{kw['va_placc'][i]:.4f}"
            row[f'thr_{l}']     = f"{kw['thresholds'][i]:.2f}"
        with open(self.path, 'a', newline='') as f:
            csv.DictWriter(f, fieldnames=self.FIELDNAMES).writerow(row)

def run_epoch(model, loader, criterion, optimizer, scaler, device: str,
              is_train: bool, thresholds=None) -> EpochResult:
    if is_train:
        model.train()
    ...
    ac_device  = device if device in ('cuda', 'cpu') else 'cpu'
    ac_enabled = (device == 'cuda')

    total_loss, all_probs, all_tgts = 0.0, [], []

    with torch.set_grad_enabled(is_train):
        for step, (images, labels) in enumerate(
                tqdm(loader, desc='Train' if is_train else 'Val  ',
                     leave=False)):
    ...
    all_probs = (np.concatenate(all_probs) if all_probs
                 else np.zeros((0, N_LABELS)))
    all_tgts  = (np.concatenate(all_tgts)  if all_tgts
                 else np.zeros((0, N_LABELS)))
...
def train_and_eval(cfg: dict, wandb_run=None):
    ...
    model = ImageModel(N_LABELS, dropout=dropout).to(DEVICE)
    ...
    scheduler               = ConstantLR(optimizer, factor=1.0,
                                         total_iters=FREEZE_UNTIL)
    unfreeze_scheduler_built = False
    scaler = GradScaler(enabled=(DEVICE == 'cuda'))

    pos_weight = compute_pos_weights(train_csv).to(DEVICE)
    criterion  = make_criterion(loss_type, focal_gamma, asl_gamma_neg,
                                label_smooth, pos_weight)


    eval_criterion = make_criterion(loss_type, focal_gamma, asl_gamma_neg,
                                    label_smooth, pos_weight=None)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if epoch <= FREEZE_UNTIL:
            ...
            if not unfreeze_scheduler_built:
                optimizer.param_groups[0]['lr'] = lr
                optimizer.param_groups[1]['lr'] = lr * lr_head_mult

                remaining    = NUM_EPOCHS - FREEZE_UNTIL
                warmup_sched = LinearLR(optimizer, start_factor=0.1,
                                        end_factor=1.0,
                                        total_iters=WARMUP_EPOCHS)
                cosine_sched = CosineAnnealingLR(
                    optimizer, T_max=remaining - WARMUP_EPOCHS, eta_min=1e-6)
                scheduler = SequentialLR(
                    optimizer,
                    schedulers=[warmup_sched, cosine_sched],
                    milestones=[WARMUP_EPOCHS])
                unfreeze_scheduler_built = True
                print(f"      → warmup {WARMUP_EPOCHS} ep "
                      f"→ cosine {remaining - WARMUP_EPOCHS} ep")

        t0 = time.time()
        tr = run_epoch(model, train_loader, criterion, optimizer,
                       scaler, DEVICE, True)
        va = run_epoch(model, val_loader, eval_criterion, None, None,
                       DEVICE, False, thresholds)

        sp, st, ep, et = split_val_for_threshold(va.probs, va.targets)
        new_thr  = find_best_thresholds(st, sp)
        va_optim = compute_metrics(et, ep, new_thr)
        ...
        if is_best:
            ...
            if epoch == 1 or epoch % 3 == 0 or epoch == NUM_EPOCHS:
                torch.save(ckpt_data,
                           ckpt_dir / f'best_ep{epoch:02d}.pth')
        ...
        csv_log.write(
            epoch=epoch, mode=mode_str, lr=cur_lr, elapsed=elapsed,
            ...
            va_f1p=va_optim.f1_each, va_auc=va_optim.aucs,
            va_placc=va_optim.per_label_acc,
            thresholds=thresholds, is_best=is_best,
        )
    ...
    if ckpt_best.exists():
        ...
        print(f"\n[Test] loaded best ckpt "
              f"(ep {ckpt.get('epoch','?')}, F1={ckpt.get('f1_macro',0):.4f})")

    te = run_epoch(model, test_loader, eval_criterion, None, None,
    ...
    try:
        ...
        if wandb_run:
            import wandb
            if test_ps:
                wandb_run.log({"test_attention": [
                    wandb.Image(str(p), caption=p.name)
                    for p in test_ps[:10]]})
            if layer_p:
                wandb_run.log({"layer_viz": wandb.Image(str(layer_p))})
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train EVA02 with default hyperparameters (no sweep).')
    parser.add_argument(
        '--mode', choices=['normal', 'supercrop', 'faceonly'], default='normal',
        help="Dataset: normal / supercrop / faceonly.")
    parser.add_argument('--loss', choices=['bce', 'focal', 'asl'],
                        default=LOSS_TYPE)
    ...
    if args.wandb:
        ...
        wb_run = wandb.init(project=WANDB_PROJECT, entity=WANDB_ENTITY,
                            name=f'baseline-{args.mode}', config=default_cfg)

    print(f"[Train] mode={args.mode}  wandb={'on' if wb_run else 'off'}")
